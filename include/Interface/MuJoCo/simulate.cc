...
void AlignAndScaleView(mj::Simulate* sim, const mjModel* m) {
  ...
  sim->cam.lookat[0] = m->stat.center[0];
  sim->cam.lookat[1] = m->stat.center[1];
  sim->cam.lookat[2] = m->stat.center[2] + 0.7;
  sim->cam.azimuth = 150;
  sim->cam.elevation = -12;
  sim->cam.distance = 1.0 * m->stat.extent;
